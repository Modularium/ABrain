...
def test_run_task_survives_learning_failures(monkeypatch):
    core = importlib.import_module("services.core")
    if hasattr(core._get_learning_state, "_state"):
        delattr(core._get_learning_state, "_state")
    registry = AgentRegistry(
        [
            AgentDescriptor(
                agent_id="adminbot-agent",
    ...
    monkeypatch.setattr(
        "core.decision.learning.online_updater.OnlineUpdater.record_execution",
        lambda self, *args, **kwargs: (_ for _ in ()).throw(RuntimeError("dataset write failed")),
    )
    ...
    assert result["execution"]["success"] is True
    assert result["feedback"]["performance"]["execution_count"] == 1
    assert result["feedback"]["warnings"] == ["learning_pipeline_failed:RuntimeError"]
    assert result["warnings"] == ["learning_pipeline_failed:RuntimeError"]
