...
def test_run_task_plan_survives_feedback_failures():
    core = importlib.import_module("services.core")
    registry = AgentRegistry(
        [
            AgentDescriptor(
    ...
    result = core.run_task_plan(
        ...
        registry=registry,
        execution_engine=FakeExecutionEngine(),
        feedback_loop=ExplodingFeedbackLoop(),
    )

    assert result["result"]["success"] is True
    assert len(result["result"]["step_results"]) == 4
    assert result["result"]["aggregated_warnings"] == [
        "feedback_loop_failed:RuntimeError",
        "feedback_loop_failed:RuntimeError",
        "feedback_loop_failed:RuntimeError",
        "feedback_loop_failed:RuntimeError",
    ]
    assert result["result"]["step_results"][0]["metadata"]["feedback_error"]["warning"] == "feedback_loop_failed:RuntimeError"
